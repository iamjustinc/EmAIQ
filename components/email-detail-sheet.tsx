...
export function EmailDetailSheet({
  ...
  return (
    ...
    >
      <SheetContent className="flex h-full w-full flex-col overflow-hidden border-l border-border bg-card p-0 text-card-foreground shadow-2xl sm:max-w-md">
        <div className="relative border-b border-border p-panel pb-6">
          <div className="mb-6 flex justify-between items-start">
            <div className="flex gap-2">
              <div className="flex items-center gap-1.5 rounded-full border border-destructive/25 bg-destructive/10 px-3 py-1 text-[9px] font-black uppercase tracking-widest text-destructive">
                <AlertCircle className="h-3 w-3" /> {email.urgency.label === 'High' ? 'Critical' : 'Priority'}
              </div>
          ...
          <h2 className="pr-10 text-2xl font-bold leading-tight tracking-tight text-foreground">{email.subject}</h2>
          <p className="mt-3 text-xs font-bold uppercase tracking-widest text-muted-foreground">
            From: <span className="ml-1 text-primary">{email.sender.name}</span>
          </p>
        </div>

        <div className="scrollbar-hide flex-1 space-y-8 overflow-y-auto p-panel pt-6">
          {sentSuccess ? (
            <div className="flex flex-col items-center justify-center py-20 animate-in zoom-in duration-300">
              ...
              <h3 className="text-lg font-bold uppercase tracking-widest text-foreground">{successMessage}</h3>
            </div>
          ) : isAnalyzing ? (
            <div className="flex flex-col items-center justify-center space-y-4 py-20">
              <Loader2 className="h-8 w-8 animate-spin text-primary" />
              <p className="text-[9px] font-bold uppercase tracking-[0.3em] text-primary">Parsing Context...</p>
            </div>
          ) : isGeneratingDraft ? (
            <div className="flex flex-col items-center justify-center space-y-6 py-20 animate-pulse">
              ...
              <p className="text-[10px] font-black uppercase tracking-[0.4em] text-primary">Synthesizing Draft...</p>
            </div>
          ) : (
            <>
              <div className="relative space-y-5 rounded-3xl border border-border bg-muted/30 p-6">
                <div className="mb-2 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-[0.25em] text-primary">Intelligence Report</span>
                  </div>
                  <Button
                    variant="ghost"
                    size="sm"
                  ...
                  >
                    <Zap className="mr-1.5 h-3 w-3 fill-primary" /> Use Draft
                  </Button>
                </div>
                <ul className="space-y-4">
                  {email.analysis?.summary?.map((point, i) => (
                    <li key={i} className="group flex gap-4">
                      <span className="mt-1 text-[10px] font-black tracking-tighter text-primary">0{i + 1}</span>
            ...
            {!isDrafting && !isDelegating ? (
              <div className="grid grid-cols-2 gap-4">
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <Button variant="outline" className="flex h-20 flex-col gap-1 rounded-2xl border-border bg-muted/40 text-muted-foreground">
                      <Clock className="h-4 w-4" />
                      <span className="text-[9px] font-bold uppercase tracking-widest">Later</span>
                    </Button>
                  </PopoverTrigger>
                  <PopoverContent side="top" className="z-[110] w-48 rounded-xl border-border bg-popover p-2 shadow-2xl">
                    <div className="flex flex-col gap-1">
                      <p className="p-2 text-[8px] font-black uppercase tracking-widest text-muted-foreground">Snooze until...</p>
                      {[
                        { label: '1 Hour', v: 1 },
                        { label: '3 Hours', v: 3 },
                        { label: 'Tomorrow', v: 24 },
              ...
              <div className="space-y-4 animate-in slide-in-from-bottom-4">
                ...
                <div className="flex gap-3">
                  <Button variant="outline" className="flex-1 border-border bg-muted/30" onClick={() => setIsDrafting(false)}>
                    Cancel
                  </Button>
                  <Button className="flex-[2] bg-primary text-primary-foreground" onClick={() => handleAction('Response Sent', 'sent')}>
                    Send Message
                  </Button>
                </div>
              </div>
